...
For a GitHub release, open the repository's **Actions** tab, run **Build and publish release** manually, and leave the version input blank. The workflow uses the committed version, builds and verifies the runtime package, attaches separate `YouTune-<version>-Chrome-Chromium.zip` and `YouTune-<version>-Microsoft-Edge.zip` files plus `SHA256SUMS.txt`, and generates release notes containing the latest commit and included commit history. The two ZIPs have equivalent runtime contents because the same Manifest V3 package serves both browser families; the names identify the intended store upload.
