...
For a GitHub release, open the repository's **Actions** tab, run **Build and publish release** manually, and leave the version input blank. The workflow uses the committed version, builds and verifies the runtime package, attaches one common `YouTune-<version>.zip` plus `SHA256SUMS.txt`, and generates release notes containing the latest commit and included commit history. The same ZIP can be uploaded separately to Chrome Web Store, Microsoft Edge Add-ons, and other compatible Chromium stores because the same Manifest V3 package serves all of them.
